...
=== Tabla compra
La tabla compra  cuya _primary key_ es el número identificario de dicha compra que consta de * 6 dígitos*, otros de los atributos de este schema es el _numero_tarjeta_  del cliente ya que el pago de la compra se
...
=== Tabla resumen
El *resumen* de la *factura* *???????????????* es cuyo documento donde se encuentran toda la información de las compras  con el _total_pagar_ . 
