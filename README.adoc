...
=== Tabla comercio
Agregamos la tabla *comercio* que consta de distintos atributos, un número identificatorio de comercio que denominamos _codigo_comercio_, el nombre, dirección y el código postal de dicho comercio.
Colocamos el código postal del comercio en caso de emitir una alerta en el momento que la tarjeta registre compras en el mismo o diferente còdigo postal de los comercios en el transcurso de un tiempo dado.
...
=== Tabla compra
La tabla *compra* cuya _primary key_ es el número identificatorio de dicha compra que consta de 3 a 4 dígitos, otros de los atributos de este schema es el _numero_tarjeta_ del cliente ya que el pago de la compra se realiza con la tarjeta de crédito. Además tenemos el atributo _horario_ que se refiere a las compras en un lapso de tiempo como ya mencionamos en la introducción.
...
Entre la informacion que incluye una factura son: el _codigo_factura_ que es la _primary key_ de la tabla, el siguiente atributo es el _dni_cliente_, además el _codigo_comercio_  que es la _foreign key_ de la tabla. 
Como toda *factura*, esta incluye _detalle_artictulo_ que tambien es una _FK_ de la tabla.
[source,sql]
----
...
----
También  añadimos las FK's de la tabla factura de la siguiente manera:
[source,sql]
----
...
=== Tabla detalle
El *detalle* de la factura generalmente se encuentra en el centro de la misma. La _primary key_ de esta tabla es _codigo_articulo_ que es el número identificatorio del articulo comprado.
Además, posee los siguientes atributos: _cantidad_ se refiere a la cantidad de artículos y el _precio_unitario_ del artículo.
[source,sql]
----
...
=== Tabla resumen
El *resumen* de la *tarjeta* es un documento donde se encuentran toda la información de las compras con el _total_pagar_ . 
La tabla *resumen* cuyas  _foreign key_ son:  _codigo_factura_, _numero_tarjeta_. Otros atributos de la tabla son: _periodo_facturado_  se refiere al tiempo desde que se inician las facturas de cada mes, hasta que finalizan es _fecha_vencimiento_. 
...
----
Añadimos las _FK's_ de tabla resumen de la siguiente manera:
[source,sql]
----
